...
export default function EarnMoney() {
  ...
  const [earnMoneyData, setEarnMoneyData] = useState({
    title: "",
    subTitle: "",
  ...
  useEffect(() => {
    if (landingPageSetup?.earnMoney) {
      setEarnMoneyData(landingPageSetup.earnMoney);
    }
  }, [landingPageSetup]);
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      setEarnMoneyData({ ...earnMoneyData, image: reader.result as string });
    };
  ...
  const handleSave = async () => {
    try {
      ...
      const updateData = {
        ...landingPageSetup,
        earnMoney: earnMoneyData,
      };
      const result = await dispatch(updateLandingPageSetup(updateData));
      if ((result as any).meta.requestStatus === "fulfilled") {
        setSubmitSuccess("Earn Money section saved successfully!");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
        <h3 className="text-lg font-bold text-gray-900 mb-6">EARN MONEY</h3>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div>
              ...
              <input
                type="text"
                value={earnMoneyData.title}
                onChange={(e) => setEarnMoneyData({ ...earnMoneyData, title: e.target.value })}
                maxLength={100}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-teal-500 outline-none"
                placeholder="Ex: Title"
              />
              <div className="text-xs text-gray-500 mt-1 text-right">{earnMoneyData.title.length}/100</div>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Sub Title</label>
              <input
                type="text"
                value={earnMoneyData.subTitle}
                onChange={(e) => setEarnMoneyData({ ...earnMoneyData, subTitle: e.target.value })}
                maxLength={200}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-teal-500 outline-none"
                placeholder="Ex: Sub Title"
              />
              <div className="text-xs text-gray-500 mt-1 text-right">{earnMoneyData.subTitle.length}/200</div>
            </div>
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-4">
              {earnMoneyData.image ? (
                <div className="relative">
                  <img
                    src={earnMoneyData.image}
                    alt="Earn money preview"
                    className="w-full h-auto rounded-lg mb-2"
                  />
                  <button
                    onClick={() => document.getElementById("earnMoneyImageUpload")?.click()}
              ...
              <div className="text-xs text-gray-500 mt-2 text-center space-y-1">
                <p>Min Size for Better Resolution 483x483 px</p>
        ...
        <div className="flex justify-end space-x-3 mt-6">
          <button
            onClick={() => setEarnMoneyData({ title: "", subTitle: "", image: null })}
